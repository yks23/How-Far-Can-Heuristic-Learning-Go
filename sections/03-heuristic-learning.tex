...
在对抗式游戏中，LLM-based coding agent 可以同时利用并弥补这两点。程序本身是高度启发性且可解释的输入；agent 进一步读取逐决策的轨迹回放来维护程序，相比只有胜负的稀疏 reward，这一信息通道更宽，能够充分利用符号程序的优势。与此同时，agent 可以持续修改同一个可执行、可读、可局部编辑的程序，逐步承担原本需要人力完成的维护工作。
