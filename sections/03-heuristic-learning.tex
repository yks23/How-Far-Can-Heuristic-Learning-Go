...
\textbf{Definition (adversarial game).} An adversarial game is an interactive
environment in which the policy's observations, transitions, or outcomes depend
on actions chosen by an external opponent. Let $\mathcal{O}$ denote the available
opponent pool; each round selects a subset $\mathcal{O}_t\subseteq\mathcal{O}$,
which determines the behaviours and feedback observed in that round.
