...
\textbf{Observation 1.} A symbolic program policy is a
...
Yet this advantage weakens when the environment changes sharply: opponent-induced
non-stationarity and distribution shift can make replay data stale, promote
overfitting to the training environment, and expose large generalization gaps
\citep{hernandezleal2019survey,zhang2018overfitting,cobbe2019quantifying,gleave2021adversarial}.
When distinct opponent modes require conflicting responses, standard RL also does
not automatically preserve a diverse set of behaviours without additional
skill-discovery or multi-task mechanisms \citep{eysenbach2019diversity,yu2020metaworld}.

\begin{center}
\fcolorbox{brandaccent}{white}{\parbox{0.93\linewidth}{\small
\textbf{Observation 2.} A symbolic
...
\begin{center}
\fcolorbox{brandaccent}{white}{\parbox{0.93\linewidth}{\small
\textbf{Observation 3.} Coding agents can understand and maintain symbolic program
policies, turning feedback into concrete revisions while reducing the human labour
required for heuristic maintenance.}}
\end{center}
...
\[
\mathcal{G}=\bigl(\mathcal{E},\boldsymbol{\Pi}\bigr),\qquad
\boldsymbol{\Pi}=\{\pi_i:\mathcal{H}_i\!\to\!\Delta(\mathcal{A}_i)\}_{i=1}^{n},\quad n\ge 2,
\]
where $\boldsymbol{\Pi}$ is a set of agent policies and $\mathcal{E}$ is the shared
environment. At each step, the participating policies jointly produce actions
$a_t^i\sim\pi_i(\cdot\mid h_t^i)$, and the environment generates the next state,
observations, and outcome according to
$\mathcal{E}(\cdot\mid s_t,a_t^1,\ldots,a_t^n)$. A round may instantiate any
subset $\boldsymbol{\Pi}_t\subseteq\boldsymbol{\Pi}$, which determines the
behaviours and feedback observed in that round.
\end{definition}
...
from matches instantiated with changing subsets of agent policies, and is evaluated
after training against a referee-side evaluation pool it cannot rewrite. AHL is therefore the adversarial-game
...
% TODO (2026-08-21): Figure 2 -- Liu Qingle and Zhao Yicheng to redraw promptly.
% Replace this qualitative placeholder with the final information-width figure.
\begin{figure}[t]
\centering
\begin{tikzpicture}[
