...
但这种优势伴随着维护代价。增加规则并不难，难的是确认它不会破坏已有行为；程序越大，每次修改的协调成本越高。瓶颈不是程序不能表达，而是持续维护最终变得过于昂贵。

\begin{center}
\fcolorbox{brandaccent}{white}{\parbox{0.93\linewidth}{\small
\textbf{观察 2：维护是瓶颈。} 启发式修改的成本随既有行为增长；当维护成本超过可投入的预算，策略改进就会停止。}}
\end{center}

在对抗式游戏中，LLM-based coding agent 可以同时利用并弥补这两点。输入端不是只有胜负这一稀疏 reward，而是包含逐决策状态、合法动作、所选动作和结果的轨迹 replay，能够指出失败发生在哪里；输出端仍是可执行、可读、可局部修改的程序。于是，agent 继承符号式范式的可解释性与编辑性，同时承担原本由人类承担的持续维护工作。

\begin{center}
\fcolorbox{brandaccent}{white}{\parbox{0.93\linewidth}{\small
\textbf{观察 3：LLM agent 的闭环作用。} LLM-based coding agent 将宽轨迹反馈转化为对可解释程序的连续修改：它利用符号式策略的优势，并弥补维护成本过高的劣势。}}
\end{center}
