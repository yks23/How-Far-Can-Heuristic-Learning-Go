...
\subsection{Arena formulation}
\label{sec:arena-formulation}

An arena instance is the tuple $(G,R,A,F,H)$: a game $G$, its fixed rules $R$,
role-specific action space $A$, referee $F$, and the archived human field $H$.
A policy reads the state visible to its role and returns an action in $A$; the referee
executes the match and returns scores and a replay. An HL run receives $(R,A)$, its
current source, and training replays, then returns a revised source. Final policies are
scored against a frozen opponent pool; the source is the only object carried between
revisions.

\benchname is an archive-backed arena of \val{ngames} competitive games, each with
fixed rules, a machine-readable action space, a runnable referee, and human-written
programs (\val{nsubv} archived versions). It is not a benchmark task collection:
the games and rankings predate our study. We add only a uniform evaluator for the
archived programs and our methods (Section~\ref{sec:arena-eval}).
...
state and returns a legal move; and an external referee scores it against opponents it
does not control. The suite includes multiplayer, non-zero-sum, partial-observation,
and asymmetric games, so no stronger game assumption is made. Each game ships its
...
The source is an annual computer-science agent contest, founded in \val{founded} and
now in its \val{editions}th edition. Each edition publishes one game's rules and kit,
runs a semester ladder, and retains every resubmitted version. The archive therefore
records trajectories of human maintenance under a real effort budget, not only final
endpoints.

The complete public asset set begins at edition~\val{edfirst}, so \benchname spans
\val{edfirst}--\val{edlast}: seven editions and \val{ngames} games. The contest
predates this benchmark and was not assembled for model evaluation, which gives a
useful contamination check rather than a proof of independence
\citep{livecodebench2024,workbuddybench2026}.

Table~\ref{tab:games} summarizes the heterogeneous suite, from grid and territory
games to survival, hidden-identity, and scripted-interrogation tasks. Appendix
~\ref{app:arena} gives the per-game archive breakdown; Appendix~\ref{app:case}
works one game's decision space and replay in full.
