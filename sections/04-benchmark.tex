...
该比赛源于由顶尖高校学生组织的经典游戏赛事，已经持续举办数十年。我们整理了
历届比赛的规则、后端、前端以及玩家提交，并从中选取九个具有代表性的游戏作为
\benchname 的样本。每个游戏的具体规则、动作空间和回放格式见附录~\ref{app:arena}；
附录~\ref{app:case} 进一步完整展示其中一个游戏的实例。

这组样本强调三个性质：\textbf{广泛性}，覆盖面向绝大多数参与者的常见游戏形式；
\textbf{经典性}，由持续数十年的赛事历史提供时间上的验证；以及\textbf{多样性}，
同时包含网格与领地争夺、生存、隐藏身份、角色不对称和脚本化推理等不同类型，
从而避免结论依赖单一游戏机制。
