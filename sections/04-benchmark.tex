...
suite departs from each of these somewhere, and Section~\ref{sec:arena-provenance} says
where. Both halves are auditable in the arena rather than asserted of it --- every game
ships a legal-move specification together with the predicate that decides it, and every
game is adjudicated by one referee contract.
...
Alongside the venue's own games we retain a set of classic games --- chess, Go, and others
whose strategic structure has an independent literature --- as structural controls. What
they control for is that our complexity measurements might validate only themselves: their
policy complexity can be argued about on external grounds, so they calibrate the proxy of
Section~\ref{sec:hl-motivation} and give the cross-game ordering a reference point our own
...
Classifying these versions by whether their strength is stored in code or in parameters
requires a criterion that decides cases, since the shares reported in
Section~\ref{sec:experiments} are worth no more than the criterion is. A submission counts
as containing a learned component when the repository ships parameters fitted from data,
or the code that fits them, for a component that affects move selection. Evaluation
coefficients a contestant tuned by hand do not count, however many matches informed the
tuning: the contestant is the optimizer in that loop, and treating human iteration as
learning would make the category include every serious entry and measure nothing. That
audit is also what makes the folk ordering of Section~\ref{sec:introduction} --- rules
lowest, search above them, learned policies highest --- a hypothesis rather than a piece
of framing, because it has a consequence the archive can check: if the ordering holds, the
paradigm composition of winning entries should climb it as budgets and tooling improve
...
across editions. Section~\ref{sec:experiments} reports what editions
\val{edfirst}--\val{edlast} say.
...
Because a policy's strength exists only relative to its opponents, every result in this
paper is an Elo rating computed from match outcomes on this referee, never an environment
return, and the archive's own ratings are treated as provenance rather than as data. Two
protocols appear and we distinguish them wherever a number is quoted: \emph{batch} Elo,
where a set of policies is rated jointly from a round-robin played after the fact, and
\emph{online} Elo, where a policy under revision is rated as it plays, so its trajectory
can be read. Ratings are comparable only within a fixed set of adversaries, so three pools
are kept apart. The \emph{training pool} is what a method may play while it improves. The
\emph{frozen test pool} is fixed before any run begins and never updated, which is what
makes ratings comparable across revisions, across methods, and across seeds. The
\emph{anchor pool}, a stratified set of archived human submissions, places ratings from
different games and different editions on one scale. Against that field two quantities are
reported and they answer different questions. A \emph{percentile} is the fraction of
recorded human versions of a game that a policy outranks --- a statement about the
distribution, and the measurement thousands of independent attempts uniquely afford, read
subject to the finalist-field caveat of Section~\ref{sec:arena-provenance}. The
\emph{human state of the art} for a game is the single strongest submission the archive
records for it, taken across editions \val{edfirst}--\val{edlast} rather than within one, a
bar that rises with every edition. Beating it is a claim about a maximum and placing within
the field is a claim about a distribution; Section~\ref{sec:experiments} reports both
...
evaluating exhaustively has saved nothing.
...
All three are recorded for every method in the common units, and additionally in whichever
unit is native to it --- environment steps for a reinforcement learner, agent invocations
for a maintenance loop --- because no single unit is native to both. Of the three only the
total may be compared across paradigms without qualification, and we say which ledger a
number comes from wherever one is quoted. Two conventions are load-bearing and we
