...
executes the match and returns scores and a replay. An AHL run receives $(R,A)$, its
...
not people: identities are incomplete and correlated, so percentiles state their unit of
independence. Only playable snapshots enter the pools; the archive's development/held-out
split is retained.

The version index supplies a common revision count for human maintenance;
Appendix~\ref{app:human} gives its distribution and caveats.

The ranked field is a finalist subset and archive ratings are provenance only; all
ratings and program-size measurements are recomputed under our contracts.
...
affects move selection; hand-tuned coefficients remain heuristic. This makes the reported
composition testable rather than dependent on how ``learning'' is named.
...
One referee interface handles all games: a seeded request fixes players and roles and
returns scores, winner, and a replay. Code is sandboxed; asymmetric roles are evaluated
in paired orders.

Player faults (illegal move, timeout, crash) count as losses; infrastructure faults are
rerun.

A replay retains each role's public state, legal actions, chosen move, and acceptance,
without exposing hidden state; this is the trace consumed by AHL (Appendix~\ref{app:case}).
...
We report Elo from this referee, not raw return. Training, frozen-test, and human-anchor
pools stay separate; the test pool is fixed before runs. Percentile measures placement in
the archived field, while the human state of the art is its strongest submission; both are
reported because they answer different questions.

Runs share one schema: candidate, reference, decision-level change, and learning,
evaluation, and total budgets. Common and native units are recorded; missing values remain
