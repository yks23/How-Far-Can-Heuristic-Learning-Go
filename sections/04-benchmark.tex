...
The suite is not uniform, and the departures constrain what may be claimed over it.
LostSpace is four-player and not zero-sum. AquaWar hides each side's roster, and
LostSpace and Rollman are partially observable, so there a replay cannot show what the
opponent knew. Rollman splits into two asymmetric roles ranked separately. DeepClue is
a single-player scripted reasoning task whose referee itself calls an external language
model, a confound for a comparison between a language-model method and reinforcement
learning, so we report it apart rather than dropping it. Where a claim needs a
