...
% SCOPE. A "What the scope excludes" subsection was written here on 2026-08-19 and cut the
% same day, at the user's direction: the scope limit is one sentence in Section 1 and one
% definition in Section 2, not a subsection. Do not reinstate it. If a referee asks what
% happens off games, the answer is the honest short one -- we did not measure it -- and it
% goes in the threats-to-validity subsection with the other limits, in a sentence.
%
% What that draft argued, kept here only so the reasoning is not lost: each requirement in
% the Section 2 definition is something the loop consumes, so tasks without them are
% harder for it, not merely different -- moving rules invalidate past revisions, an absent
% referee makes an improvement claim unfalsifiable, a model as judge can be optimised, and
% with no opponent the signal is exhaustible. That makes games the favourable case, which
% is why a limit found here is evidence about the agent rather than about the task. The
% clause in Section 1 carries the conclusion; the argument was the part that did not earn
% its page.
\section{Discussion}
